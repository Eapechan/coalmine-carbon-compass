...
const Reports = () => {
  ...
  const avgOffsets = carbonSinks.length > 0 ? carbonSinks.reduce((sum, s) => sum + s.amount, 0) / carbonSinks.length : 0;
  const maxEmission = emissionEntries.length > 0 ? Math.max(...emissionEntries.map(e => e.co2e)) : 0;
  const maxOffset = carbonSinks.length > 0 ? Math.max(...carbonSinks.map(s => s.amount)) : 0;
  ...
  const handleExportPDF = async () => {
    ...
    doc.text("CoalMine Carbon Compass", pageWidth - 60, 80, { align: "right" });
